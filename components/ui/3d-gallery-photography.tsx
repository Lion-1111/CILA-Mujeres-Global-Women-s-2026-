...
const MAX_HORIZONTAL_OFFSET = 5;
const MAX_VERTICAL_OFFSET = 4;

const u = (m: THREE.ShaderMaterial) =>
  m.uniforms as Record<string, { value: unknown } | undefined>;
...
function ImagePlane({
  ...
  useEffect(() => {
    if (material && texture) u(material)["map"]!.value = texture;
  }, [material, texture]);

  useEffect(() => {
    if (material?.uniforms) u(material)["isHovered"]!.value = isHovered ? 1.0 : 0.0;
  }, [material, isHovered]);
  ...
  useFrame((state, delta) => {
    ...
    planesData.current.forEach((plane, i) => {
      ...
      if (material?.uniforms) {
        u(material)["opacity"]!.value = opacity;
        u(material)["blurAmount"]!.value = blur;
      }
    });
  ...
  return (
    <>
      {planesData.current.map((plane, i) => {
        ...
        const worldZ = plane.z - depthRange * 0.43;
  ...
  return (
    <div className="h-full w-full overflow-auto p-6">
      <p className="mb-4 text-sm text-muted-foreground">
        WebGL no disponible. Mostrando galería estática:
      </p>
